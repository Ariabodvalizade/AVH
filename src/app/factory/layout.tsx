    ...
    <div data-layer="factory">
      ...
      <header className="glass sticky top-0 z-[var(--z-nav)]">
        <nav className="mx-auto flex max-w-7xl items-center justify-between gap-6 px-6 py-4">
          <Link href="/factory" className="flex items-baseline gap-2 font-bold">
          ...
          <ul className="hidden items-center gap-6 text-sm md:flex">
            {factoryNav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="text-muted transition-colors duration-[var(--duration-fast)] hover:text-paper"
                >
                  {item.label}
                </Link>
              </li>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-[2fr_1fr]">
          <div>
            <p className="font-extrabold" style={{ fontSize: "var(--text-h3)" }}>
              پروژهٔ صنعتی دارید؟
            </p>
            <p className="mt-2 max-w-[50ch] text-muted">
              از یک کاتالوگ AR تا استقرار عینک روی خط تولید — بگویید چه می‌خواهید تا امکان‌سنجی
              کنیم.
            </p>
          ...
          <div className="text-sm text-muted">
            ...
            <p className="mt-6">{siteSettings.brandFa} © ۱۴۰۴</p>
          </div>
        </div>
      </footer>
    </div>
